...
void ChunkManager::draw() {
	...
	for(auto& chunk : sortedChunks) {
		_chunkShader->setVec3("chunkWorldCoord", glm::vec3(chunk->worldCoord.x, 0.f, chunk->worldCoord.z));
		chunk->drawSolid();
	}
...
void ChunkManager::_setNearbyChunksMinMax(Chunk* chunk, const uint8_t& min, const uint8_t& max) {
	auto setMinMax = [&](Chunk* nearbyChunk) {
		if(nearbyChunk != nullptr) {
			nearbyChunk->minimumPoint = std::min(nearbyChunk->minimumPoint, (min < 0) ? static_cast<uint8_t>(0) : min);
			nearbyChunk->highestPoint = std::max(nearbyChunk->highestPoint, (max > CHUNK_HEIGHT - 1) ? static_cast<uint8_t>(CHUNK_HEIGHT - 1) : max);
		}
	};
