...
void WorldGenerator::_generateHeightMap() {
	...
	for(int z = 0; z < CHUNK_SIZE; z++) {
		const int heightValue = _biomeMap.get(x, z)->getHeight(x, z, _chunk->coord.x, _chunk->coord.z);

		_chunk->minimumPoint = std::min(_chunk->minimumPoint, uint8_t(heightValue));
		_chunk->highestPoint = std::max(_chunk->highestPoint, uint8_t(heightValue + 10));
...
void WorldGenerator::_setBlocks() {
	...
	for(int z = 0; z < CHUNK_SIZE; z++) {
		...
		uint8_t height = _heightMap.get(x, z);
