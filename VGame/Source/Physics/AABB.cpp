...
void AABB::collision(Player& player, const glm::vec3& velocity) {
	...
	for(int z = player.position.z - dimensions.z; z < player.position.z + dimensions.z; z++) {
		...
		Block* block = BlockManager::blocks[World::getChunkManager().getChunkBlock({ x, y, z }).blockID];
		if(block->hasHitbox) {
			if(velocity.x > 0) {
...
void AABB::collision(Particle& particle) {
	...
	if(BlockManager::blocks[World::getChunkManager().getChunkBlock({ int(pos.x + vel.x), int(pos.y), int(pos.z) }).blockID]->hasHitbox) {
		particle.position.x = int(pos.x);
		particle.velocity.x = 0;
	}

	if(BlockManager::blocks[World::getChunkManager().getChunkBlock({ int(pos.x), int(pos.y + vel.y), int(pos.z) }).blockID]->hasHitbox) {
	...
	if(BlockManager::blocks[World::getChunkManager().getChunkBlock({ int(pos.x), int(pos.y), int(pos.z + vel.z) }).blockID]->hasHitbox) {
